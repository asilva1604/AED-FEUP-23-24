...
vector<T> Graph<T>::bfs(const T & source) const {
	...
	vector<T> res;
    queue<Vertex<T> *> waitingToBeVisited;
    bool flag = false;
    for (const auto &v : vertexSet) {
        if (v->getInfo() == source) {
            flag = true;
        ...
        if (!flag) continue;
        waitingToBeVisited.push(v);
        v->setVisited(true);
        while (!waitingToBeVisited.empty()) {
            auto node = waitingToBeVisited.front();
            waitingToBeVisited.pop();
            for (const auto &e : node->getAdj()) {
                if (!e.getDest()->isVisited()) {
                    waitingToBeVisited.push(e.getDest());
